...
public class View2dContainer extends ImageViewerPlugin<DicomImageElement> implements PropertyChangeListener {
  ...
  public static final List<SynchView> DEFAULT_SYNCH_LIST = Arrays.asList(SynchView.NONE, SynchView.DEFAULT_STACK,
      SynchView.DEFAULT_TILE);

  public static final GridBagLayoutModel VIEWS_2x1_r1xc2_dump = new GridBagLayoutModel(
      View2dContainer.class.getResourceAsStream("/config/layoutModel.xml"), "layout_dump", // NON-NLS
  ...
  // Static tools shared by all the View2dContainer instances, tools are
  // registered when a container
  // is selected
  // Do not initialize tools in a static block (order initialization issue with
  // eventManager), use
  // instead a lazy
  ...
  public View2dContainer(GridBagLayoutModel layoutModel, String uid, String pluginName, Icon icon, String tooltips) {
    ...
    addComponentListener(new ComponentAdapter() {
      ...
      public void componentResized(ComponentEvent e) {
        ...
        if (container == View2dContainer.this) {
          Optional<ComboItemListener> layoutAction = EventManager.getInstance().getAction(ActionW.LAYOUT,
              ComboItemListener.class);
    ...
    if (!initComponents) {
      ...
      if (InsertableUtil.getBooleanProperty(BundleTools.SYSTEM_PREFERENCES, bundleName, componentName,
        ...
        Optional<Toolbar> b = UIManager.EXPLORER_PLUGIN_TOOLBARS.stream().filter(t -> t instanceof ImportToolBar)
            .findFirst();
        b.ifPresent(TOOLBARS::add);
      }
      if (InsertableUtil.getBooleanProperty(BundleTools.SYSTEM_PREFERENCES, bundleName, componentName,
          InsertableUtil.getCName(ExportToolBar.class), key, true)) {
        Optional<Toolbar> b = UIManager.EXPLORER_PLUGIN_TOOLBARS.stream().filter(t -> t instanceof ExportToolBar)
            .findFirst();
        b.ifPresent(TOOLBARS::add);
      }
      if (InsertableUtil.getBooleanProperty(BundleTools.SYSTEM_PREFERENCES, bundleName, componentName,
        ...
        TOOLBARS.add(
            new ViewerToolBar<>(evtMg, evtMg.getMouseActions().getActiveButtons(), BundleTools.SYSTEM_PREFERENCES, 10));
      }
      if (InsertableUtil.getBooleanProperty(BundleTools.SYSTEM_PREFERENCES, bundleName, componentName,
      ...
      if (InsertableUtil.getBooleanProperty(BundleTools.SYSTEM_PREFERENCES, bundleName, componentName,
          InsertableUtil.getCName(EndoDataToolBar.class), key, true)) {
        TOOLBARS.add(new EndoDataToolBar(evtMg, 45));
      }
      if (InsertableUtil.getBooleanProperty(BundleTools.SYSTEM_PREFERENCES, bundleName, componentName,
  ...
  public void setSelectedImagePaneFromFocus(ViewCanvas<DicomImageElement> viewCanvas) {
    ...
    if (viewCanvas != null && viewCanvas.getSeries() instanceof DicomSeries) {
      ...
      DicomSeries.startPreloading(series,
          series.copyOfMedias((Filter<DicomImageElement>) viewCanvas.getActionValue(ActionW.FILTERED_SERIES.cmd()),
  ...
  public void setSelected(boolean selected) {
    if (selected) {
      ...
      if (dicomView != null && dicomView.getDataExplorerModel() instanceof DicomModel) {
        dicomView.getDataExplorerModel()
            .firePropertyChange(new ObservableEvent(ObservableEvent.BasicAction.SELECT, this, null, getGroupID()));
      }
      ...
      if (newVal instanceof SeriesEvent) {
                ...
                if (img != null && view2DPane.getSeries() == series) {
                  ...
                  if (seqAction instanceof SliderCineListener) {
                    ...
                    if (param instanceof DicomImageElement) {
                      ...
                      int imgIndex = series.getImageIndex(img, filter, view2DPane.getCurrentSortComparator());
                      if (imgIndex < 0) {
                        imgIndex = 0;
        ...
        if (newVal instanceof Series) {
          ...
          for (ViewCanvas<DicomImageElement> v : view2ds) {
            ...
            if (series.equals(s)) {
              /*
               * Set to null to be sure that all parameters from the view are apply again to
               * the Series (for instance it is the same series with more images)
               */
              v.setSeries(null);
        ...
        // When a dicom KO element is loaded an ObservableEvent.BasicAction.Update is
        // sent
        // Either it's a new DicomObject or it's content is updated

        // TODO - a choice should be done about sending either a DicomSpecialElement or
        // a Series
        // object as the
        // new value for this event. A DicomSpecialElement seems to be a better choice
        // since a
        // Series of
        // DicomSpecialElement do not necessarily concerned the series in the
        // Viewer2dContainer
            ...
            if (view instanceof View2d) {
              ...
              if (img != null) {
                if (PresentationStateReader.isModuleAppicable(
                    TagD.getTagValue(specialElement, Tag.ReferencedSeriesSequence, Attributes[].class), img)) {
  ...
  private void setKOSpecialElement(KOSpecialElement updatedKOSelection, Boolean enableFilter, boolean forceUpdate,
    ...
    if (updatedKOSelection != null && selectedView instanceof View2d) {
      if (SynchData.Mode.TILE.equals(this.getSynchView().getSynchData().getMode())) {
        ...
        if (forceUpdate || enableFilter != null) {
          ...
          if (koFilterAction instanceof ToggleButtonListener) {
            if (enableFilter == null) {
              enableFilter = LangUtil.getNULLtoFalse((Boolean) selectedView.getActionValue(ActionW.KO_FILTER.cmd()));
            }
        ...
        /*
         * Set the selected view at the end of the list to trigger the synchronization
         * of the SCROLL_SERIES action at the end of the process
         */
        List<ViewCanvas<DicomImageElement>> viewList = getImagePanels(true);
